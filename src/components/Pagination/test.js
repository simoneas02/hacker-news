...
import Pagination, { pagination } from '.';
...
describe('Pagination rules', () => {
  ...
  test('pagination() should return [1]', () => {
    const result = [1];
    assert(pagination()).to.be.deep.equal(result);
  });
  ...
  test('pagination({ total: "abc", activePage: 1 }) should throw an error', () => {
    ...
    try {
      ...
      assert(e.message).to.be.equal(result);
    }
  });
